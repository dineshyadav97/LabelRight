...
public class ResultInterruptionFlow {
	...
	public void Flow6(int regionOption, int packageType, String artworkUrl, String lidUrl) throws InterruptedException {
		...
		/*
		 * try { WebElement upload_page_next = new
		 ...
		 * log.info("Upload Button CLicked."); } catch (Exception e) {
		 * log.error("Error in UploadPage"); log.error(e); }
		 * 
		 * try { String loading_container_xpath =
		 * "/html/body/app-root/app-page-loading/div/app-loading-spinner/div/div";
		 * Boolean loading_container_element = new WebDriverWait(driver,
		 * Duration.ofSeconds(400)).until(ExpectedConditions.
		 * invisibilityOfElementLocated(By.xpath(loading_container_xpath))); String
		 * verify_page_next_xpath =
		 * "/html/body/app-root/main/app-verify/div/div/div/div[1]/app-side-button-panel/div/div[1]/div[2]/div[1]/button";
		 * WebElement verify_page_next = new
		 * WebDriverWait(driver,Duration.ofSeconds(100)).until(ExpectedConditions.
		 * elementToBeClickable(By.xpath(verify_page_next_xpath)));
		 * js.executeScript("arguments[0].click();", verify_page_next);
		 * 
		 * 
		 * log.info("Verify Button Clicked."); } catch (Exception e) {
		 * log.error("Error in VerifyPage"); log.error(e); }
		 */
		...
		WebElement result_page = wait.until(
				ExpectedConditions.elementToBeClickable(By.xpath("/html/body/app-root/main/app-results/div/div")));

		js.executeScript("arguments[0].click();", result_page);
		log.info("entered result page");
